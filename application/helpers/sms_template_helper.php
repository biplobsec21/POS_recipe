				...
				switch ($template_id) {
					case 1:
						...
						if ($q2->num_rows() > 0 && $q2->row()->customer_id !== 1 && !empty($q2->row()->mobile)) {
							...
							$content = str_replace("{{invoice_due_amt}}", $CI->currency_code(number_format($q2->row()->grand_total - $q2->row()->paid_amount, 2, '.', '')), $content);
							$content = str_replace("{{cust_tot_due_amt}}", $CI->currency_code(number_format($q2->row()->sales_due, 2, '.', '')), $content);
