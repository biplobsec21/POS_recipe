...
class Reports_model extends CI_Model
{
	...
	public function show_sales_return_report()
	{
		...
		$this->db->from("db_customers as b");
		$this->db->select("CASE WHEN c.sales_code IS NULL THEN '' ELSE c.sales_code END AS sales_code,a.sales_id");
	...
	public function show_item_sales_report()
	{
		...
		if ($q1->num_rows() > 0) {
			...
			echo "<tr>
					  <td class='text-right text-bold' colspan='5'><b>Total :</b></td>
					  <td class='text-left text-bold'>" . app_number_format($tot_sales_qty) . "</td>
	...
	public function show_item_purchase_report()
	{
		...
		if ($q1->num_rows() > 0) {
			...
			echo "<tr>
					  <td class='text-right text-bold' colspan='5'><b>Total :</b></td>
					  <td class='text-left text-bold'>" . app_number_format($tot_purchase_qtyt) . "</td>
					  <td class='text-right text-bold'>" . app_number_format($tot_total_cost) . "</td>
				  </tr>";
		} else {
			echo "<tr>";
	 ...
	 */
	/**
	 * Get production report data - FIXED VERSION
	 */
	/**
	 * Get production report data - USING RAW SQL
	 */
	/**
	 * Get production report data - CORRECTED FOR ACTUAL SCHEMA
	 */
	/**
	 * Get production report data - FIXED DATE CONVERSION
	 */
	public function get_production_report()
	{
		...
		error_log("Production Report Filter - from_date: $start_date, to_date: $end_date, status: $status");

		// Build WHERE conditions
		$where_conditions = array();
		$params = array();

		// Apply date filter - FIXED DATE CONVERSION
		if (!empty($start_date) && !empty($end_date)) {
			// Convert dd-mm-yyyy to Y-m-d - PROPER CONVERSION
			...
			if (!$start_date_mysql) {
				...
				$start_date_mysql = $start_date_parts[2] . '-' . $start_date_parts[1] . '-' . $start_date_parts[0];
			}
			if (!$end_date_mysql) {
				$end_date_parts = explode('-', $end_date);
				$end_date_mysql = $end_date_parts[2] . '-' . $end_date_parts[1] . '-' . $end_date_parts[0];
			}

			error_log("Date filter - start: $start_date_mysql, end: $end_date_mysql");

			$where_conditions[] = "DATE(pb.created_at) >= ?";
			$where_conditions[] = "DATE(pb.created_at) <= ?";
		...
		$sql = "SELECT 
                ...
                r.yield_quantity, 
                pb.batch_quantity,
                (r.yield_quantity * pb.batch_quantity) as output_quantity,
                pb.total_cost, 
                pb.cost_per_unit, 
                pb.created_at, 
                pb.status,
            ...
            LEFT JOIN db_users u ON u.id = pb.created_by
            $where_clause
            ORDER BY pb.created_at DESC";

		error_log("Production Report SQL: " . $sql);
		...
		if ($query === FALSE) {
			$error = $this->db->error();
			error_log("Database Query Failed: " . $error['message']);
		...
		error_log("Production Report Rows Found: " . $query->num_rows());
		...
		if ($query->num_rows() > 0) {
			...
			foreach ($result as $row) {
				$data[] = array(
					...
					'batch_quantity' => number_format($row->batch_quantity, 2),
					'output_quantity' => number_format($row->output_quantity, 2),
			...
			return json_encode(array(
				'success' => true,
				'data' => array(),
				'message' => 'No production batches found for the selected criteria',
				'debug' => array(
					'query' => $sql,
					'params' => $params
				)
	...
	/**
	 * Export production report - UPDATED for actual schema
	 */
	/**
	 * Export production report - CORRECTED FOR ACTUAL SCHEMA
	 */
	/**
	 * Export production report - FIXED DATE CONVERSION
	 */
	public function export_production_report($type = 'excel')
	{
		...
		if ($query === FALSE) {
			show_error('Database query failed: ' . $this->db->error()['message']);
		}

		$results = $query->result();

		// Load library based on export type
		if ($type == 'excel') {
			$this->export_to_excel($results);
		} elseif ($type == 'pdf') {
			$this->export_to_pdf($results);
		} else {
			$this->export_to_print($results);
		}
	}
}
