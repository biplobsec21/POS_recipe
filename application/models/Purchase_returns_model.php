...
class Purchase_returns_model extends CI_Model
{
	...
	function update_purchase_payment_status_by_purchase_id($return_id, $supplier_id)
	{
		...
		$payment_status = '';
		if ((float)$payble_total === (float)$sum_of_payments) {
			$payment_status = "Paid";
		} else if ((float)$sum_of_payments !== 0.0 && ((float)$sum_of_payments < (float)$payble_total)) {
			$payment_status = "Partial";
		} else if ((float)$sum_of_payments === 0.0) {
...
?>
		<tr id="row_<?= $rowcount; ?>" data-row='<?= $rowcount; ?>'>
			...
			<td id="td_<?= $rowcount; ?>_5" class="<?= tax_disable_class() ?>">
				<input type="text" name="td_data_<?= $rowcount; ?>_5" id="td_data_<?= $rowcount; ?>_5" class="form-control text-right no-padding only_currency text-center" readonly value="<?= $item_tax_amt; ?>">
			</td>
