...
class Customers_model extends CI_Model
{
	...
	// Calculate actual sales due based on sales and payments
	...
	// Get calculated sales due for datatable
	public function get_calculated_sales_due($customer_id)
	{
		$result = $this->calculate_actual_sales_due($customer_id);
		return $result ? $result->calculated_sales_due : 0;
	}

	// Repair customer sales due - update stored value with calculated value
	public function repair_customer_sales_due($customer_id = null)
	{
		if ($customer_id) {
			// Repair specific customer
			$result = $this->calculate_actual_sales_due($customer_id);
			if ($result) {
				$this->db->where('id', $customer_id);
				$this->db->update('db_customers', array(
					'sales_due' => $result->calculated_sales_due
				));
				return "Customer ID {$customer_id} sales due updated from {$result->stored_sales_due} to {$result->calculated_sales_due}";
			}
			return "No data found for customer ID {$customer_id}";
		} else {
			// Repair all customers
			$customers = $this->db->where('status', 1)->get('db_customers')->result();
			$updated_count = 0;

			foreach ($customers as $customer) {
				$result = $this->calculate_actual_sales_due($customer->id);
				if ($result) {
			...
			return "{$updated_count} customer sales due amounts have been recalculated and updated.";
		}
	}
	...
	public function delete_customers_from_table($ids)
	{
		...
		if ($q1->row()->tot_entrys > 0) {
			echo "Sales Invoices Exist of Customer! Please Delete Sales Invoices!";
			exit();
		}
	...
	public function show_pay_now_modal($customer_id)
	{
		...
		$customer_sales_due = $this->get_calculated_sales_due($customer_id);
...
<?php
	...
	public function get_total_due_amount($customer_id)
	{
		// Use calculated sales due instead of stored value
		$calculated_sales_due = $this->get_calculated_sales_due($customer_id);

		$q2 = $this->db->query("select * from db_customers where id=?", array($customer_id));
		$res2 = $q2->row();

		$customer_opening_balance = $res2->opening_balance;
		$sum_of_ob_paid = $this->db->query("select coalesce(sum(payment),0) sum_of_ob_paid from db_cobpayments where customer_id=?", array($customer_id))->row()->sum_of_ob_paid;
		$customer_opening_balance_due = $customer_opening_balance - $sum_of_ob_paid;

		$due_amount = number_format($calculated_sales_due + $customer_opening_balance_due, 2, '.', '');
	...
	// Get total paid amount for customer
	...
	public function get_customer_financial_summary($customer_id)
	{
		$summary = array();

		// Customer basic info
		$customer = $this->get_customer_details($customer_id);
		$summary['customer_info'] = $customer;

		// Sales due calculation
		$sales_due_info = $this->calculate_actual_sales_due($customer_id);
		$summary['sales_due_info'] = $sales_due_info;

		// Opening balance calculations
		$sum_of_ob_paid = $this->db->query("select coalesce(sum(payment),0) sum_of_ob_paid from db_cobpayments where customer_id=?", array($customer_id))->row()->sum_of_ob_paid;
		$summary['opening_balance_due'] = $customer->opening_balance - $sum_of_ob_paid;
		$summary['total_cob_payments'] = $sum_of_ob_paid;

		// Total due
		$summary['total_due'] = $this->get_total_due_amount($customer_id);
