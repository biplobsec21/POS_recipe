...
?>
	...
	?>
		...
		?>
					<!--<div style="height:1.4in !important; line-height: 1in; width:3.6375in !important; display: block;  " class="label_border text-center">-->
					...
					<!--	<img class="center-block" style="max-height: 0.81in !important; width: 100%; opacity: 1.0" src="<?php echo base_url(); ?>barcode/<?php echo $item_code; ?>">-->
					<!--    <div style="max-heigth:29mm !important;text-align:center;">-->
					<!--<span style="width:42mm;overflow:hidden;height:18px;"><?= $item_name; ?></span><br />-->
					<!--<img style="width:42mm" src="<?php echo base_url(); ?>barcode/<?php echo $item_code; ?>"><br />-->

					<!--<span><?= $item_code ?> &nbsp;&nbsp; &nbsp;  <?= $CI->currency($item_price); ?></span><br />-->
					<!--</div>-->
					<!--</div>-->
					<!--</div>-->

					<p style="max-heigth:29mm !important;text-align:center;">
						<img style="width:42mm" src="<?php echo base_url(); ?>barcode/<?php echo $item_code; ?>"><br />
						<span style="font-size:11px;"><?= $item_code ?> &nbsp;&nbsp; &nbsp; <?= $CI->currency($item_price); ?></span><br />
						<span style="font-size:10px;"><?= $item_name; ?></span><br />
					</p>
		<?php
				}
			}
...
<?php
	...
	{
		$this->db->select('item_name, purchase_price, stock, (SELECT unit_name FROM db_units WHERE id=db_items.unit_id) as unit_name');
		$this->db->from('db_items');
		$this->db->where('id', $id);
		$query = $this->db->get();
		return $query->row();
	...
	{
		$this->db->select('a.id, a.item_name, a.item_code, a.stock, a.purchase_price, c.unit_name');
		$this->db->from('db_items as a');
		$this->db->join('db_units as c', 'c.id=a.unit_id', 'left');
		$this->db->where("(a.item_name LIKE '%$term%' ESCAPE '!' OR a.item_code LIKE '%$term%' ESCAPE '!')");
	...
	/**
	 * Get item details by id (used by recipe edit view)
	 * returns object or null
	 */
	public function get_by_id($id)
	{
		$this->db->select('a.id as id, a.item_name, a.item_code, a.stock, a.purchase_price, a.unit_id, c.unit_name');
		$this->db->from('db_items as a');
		$this->db->join('db_units as c', 'c.id = a.unit_id', 'left');
